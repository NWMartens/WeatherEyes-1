    ...
    def Forecast_Inquiry(self, date, time, service_prov, timeinterval):
        ...
        if timeinterval == "daily":
            for x in results:
                ...
                Docs = {"Date" : x["Date"], "Precipitation Probability" : x["PrecipitationProb"] , "Description" : x["Description"], "MinTemp" : x["MinTempF"], "MaxTemp" : x["MaxTempF"]} # Will eventually code to user preference
        ...
        if timeinterval == "hourly":
            if service_prov == "AccuWeather":
                for x in results:
                    ...
                    Descrip.append(x["Description"])
                    Docs = {"Hour" : x["Hour"], "Precipitation Probability" : x["PrecipitationProb"], "Description" : x["Description"], "Temperature" : x["TemperatureF"]}
                    Listionary.append(Docs)
                return Listionary
            if service_prov == "NatWeather":
                for x in results:
